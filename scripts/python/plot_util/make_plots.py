...
def make_plots(argv):
	...
	nplots = len([k for v in data.values() for k in v.keys()])
	if ( nplots == 0 ):
	...
	for target_idx, target in enumerate(targets, start=1):
		...
		for inpath_idx, inpath in enumerate(inpaths):
			...
			xvar_data, yvar_data = [], []
			xvar_data = [ float(score[xvar_idx]) 
				for score in data[inpath][target].scores
				if not options.ignore_missing or (score_labels.index("missing") > -1 and
				float(score[score_labels.index("missing")])) == 0 ]
			yvar_data = [ float(score[yvar_idx]) 
				for score in data[inpath][target].scores
				if not options.ignore_missing or (score_labels.index("missing") > -1 and
				float(score[score_labels.index("missing")])) == 0 ]
			#for score in data[inpath][target].scores:
			...
			title_fontsize = 'small' if nrows < 3 else 16#8
			ax.set_title( get_title(target), fontsize=title_fontsize, weight='bold', horizontalalignment='center' )
			ax.set_ylabel(string.join(ylabels, ', '), fontsize=16 )#6 )
			ax.set_xlabel(string.join(xlabels, ', '), fontsize=16 )#6 )
			for ticklabel in ax.yaxis.get_ticklabels()+ax.xaxis.get_ticklabels():
				ticklabel.set_fontsize(12)#8)
			ax.set_xlim(0, 16)
			...
			# Skip for now -- there is a problem with these axes and we are going to 
			# delete it anyway.
		...
		# We need to set the y limits COLLECTIVELY -- taking each subplot
		# into account.	
		xvar_data, yvar_data = {}, {}
		for inpath in inpaths:
			print inpath
			xvar_data[inpath] = [ float(score[xvar_idx]) 
				for score in data[inpath][target].scores
				if not options.ignore_missing or (score_labels.index("missing") > -1 and
				float(score[score_labels.index("missing")])) == 0 ]
			yvar_data[inpath] = [ float(score[yvar_idx]) 
				for score in data[inpath][target].scores
				if not options.ignore_missing or (score_labels.index("missing") > -1 and
				float(score[score_labels.index("missing")])) == 0 ]
		ymin, ymax = min([min(yd) for yd in yvar_data.values()]), max([max(yd) for ip, yd in yvar_data.items() if "ALL_FARFAR" not in ip])
		ymax = max([ymax, min(yvar_data["/home/amw579/stepwise_benchmark/new/ALL_FARFAR"])+4])
...
def init_options_parser():
	...
	parser.add_argument(
		...
		default=['swm_rebuild_full_model.out',
			 'swm_rebuild.out',
			 'swm_rebuild.sc',
			 'farna_rebuild.sc',
			 'farna_rebuild.out',
